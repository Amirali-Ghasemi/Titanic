...
# این فانکشن ظاهر کلی صفحه (عنوان تب، آیکن، layout) رو تعیین می‌کنه
st.set_page_config(
...
#    بخش هدر اصلی اپلیکیشن؛ همیشه بالای صفحه دیده می‌شه
st.markdown(
    """
...
# این قسمت مدل ذخیره شده در پوشه models رو می‌خونه
...
@st.cache_resource
def load_model():
    # اگر فایل مدل وجود نداشت، ارور کاربرپسند می‌دم
    if not os.path.exists(MODEL_PATH):
        st.error(
            "Model file not found! Please train the model and save it to the 'models/voting_clf_model.pkl' path."
        )
        return None
...
#    این بخش ظاهر سایدبار سمت چپ رو می‌سازه
with st.sidebar:
    ...
    fare = st.number_input(
        "Fare Paid ($)", min_value=0.0, max_value=512.0, value=34.5, step=0.5
    )
...
# این DataFrame دقیقا ورودی خام کاربره
...
# این قسمت باید دقیقا همون فیچرهایی رو بسازه که تو نوت‌بوک training.ipynb ساختم
...
#    این بخش وسط صفحه رو به سه ستون تقسیم می‌کنه:
# ۱. گیج احتمال بقا
...
try:
    ...
    with col_gauge:
        ...
        #    برای حس گیج، از یک نوار پیشرفت افقی و عدد درصد استفاده می‌کنم
        st.markdown(
            f"""
    ...
    with col_feat_imp:
        ...
        feat_df = pd.DataFrame({"Feature": feat_names, "Importance": feat_values})
    ...
    with col_prob_dist:
        ...
        st.caption(
            "Synthetic distribution around current prediction (for visualization)."
        )
    ...
    # این بخش خلاصه ورودی‌ها رو در قالب متن مرتب نشان می‌دهد
    profile_text = (
        f"Age {age}, "
        f"{sex.capitalize()}, "
        f"Class {pclass}, "
    ...
    #    اگر هر اروری در زمان predict ایجاد شد، اینجا نمایش داده می‌شه
